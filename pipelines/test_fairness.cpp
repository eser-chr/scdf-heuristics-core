...
void write_csv_results(const fs::path &output_path, const std::vector<RES> &results)
{
    ...
    out << "path,N,method,duration,objective,min,max,mean,std\n";
...
stats calc_stats(const std::vector<double> &v)
{
    ...
    double s = std::accumulate(v.begin(), v.end(), 0.0, [m](double a, size_t x)
                               { return a + (x - m) * (x - m); });
        ...
        for (auto fair_factor : fairness)
        {
            ...
            for (auto const &instance : instance_paths)
            {
                ...
                { // Beam Search
                    ...
                    std::cout << " bs";
                    Timer t;
                    auto bs_sol = BS::beam_search(I, 1.0, 20); // after tuning
                    double exec_time = t.get_time();
